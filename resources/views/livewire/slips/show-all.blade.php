            ...
            <table class="min-w-full w-full divide-y divide-gray-200 mb-3">
                <thead>
                    <tr>
                        ...
                        <th class="w-1/5 px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Početno stanje</th>
                        <th class="w-1/5 px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Primljeno</th>
                        <th class="w-1/5 px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Stanje</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($slips as $slip)
                        <tr wire:key="slip_{{$slip->id}}">
                            <td class="px-6 py-4 text-sm whitespace-no-wrap">{{$slip->formatted_slip_date}}</td>
                            <td class="px-6 py-4 text-sm whitespace-no-wrap">{{$slip->formatted_status_start}} din.</td>
                    ...
                    @empty 
                        <tr>
                            <td class="px-6 py-4 text-sm whitespace-no-wrap" colspan="6">Nemate nijedan slip ili ne postoji slip sa unetom pretragom.</td>
                        </tr>
                    @endforelse
                    </tbody>
            </table>
            {{ $slips->links('pagination.custom-pagination') }}
        </div>
    </div>
</div>